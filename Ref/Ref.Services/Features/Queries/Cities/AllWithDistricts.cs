    ...
    public class AllWithDistricts
    {
        ...
        public class Handler : IRequestHandler<Query, Result>
        {
            ...
            public async Task<Result> Handle(Query request, CancellationToken cancellationToken)
            {
                try
                {
                    var entities = await _citiesRepository.FindByAsync(c => c.HasDistricts);
                    ...
                    else return new Result { Message = "No cities with districts in database" };
                }
                catch (Exception ex)
                {
